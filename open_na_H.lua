...
function main()
    ...
    downloadUrlToFile("https://github.com/Juonior/update", thisScript().path, function(id, status)
      ...
      sampAddChatMessage("{00ff85}[MVDHelper Jun v2]: {d0ffe9}ѕриложение успешно запущено. –азработчик приветствует вас.", 0x00ff85)
    while true do
        wait(0)
...
function sampev.onServerMessage(color, text)
    if text:match('^%[R%] (.*) (.*)%[(%d+)%]: (.*)') then -- покраска ников в /r, /f чатах
